...
async function dump() {
  ...
  const [rows]: any = await poolConnection.query("SELECT id, name, html FROM email_layout WHERE id = 'inbox'");
  if (rows.length > 0) {
    fs.writeFileSync('/tmp/inbox_layout.html', rows[0].html);
    console.log('✅ Exported to /tmp/inbox_layout.html');
  }
  process.exit(0);
}
